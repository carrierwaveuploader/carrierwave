...
class RenameUploader < CarrierWave::Uploader::Base
  def store_dir
    "uploads/#{model.folder_was || model.folder}"
  end
  def stale_model?
    model.folder_changed?
  end
end
...
describe CarrierWave::Mongoid do
  ...
  describe "#save" do
    ...
    context "when the store_dir was changed" do
      ...
      it 'should change the folder' do
        ...
        File.exist?(public_path('uploads/trash/test.jpg')).should be_true
      end

      it 'should delete the original file' do
        @doc.folder = 'trash'
        @doc.image = stub_file('portrait.jpg')
        @doc.save
        File.exist?(public_path('uploads/test.jpeg')).should be_false
        File.exist?(public_path('uploads/trash/portrait.jpg')).should be_true
      end
