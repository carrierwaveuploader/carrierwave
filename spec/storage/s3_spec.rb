...
if ENV['REMOTE'] == 'true'
  describe CarrierWave::Storage::S3 do
    ...
    describe '#store!' do
      ...
      context "with custom public cname" do
        it "should have a public URL set to the cname" do
          @uploader.stub!(:s3_cnamed).and_return(false)
          @uploader.stub!(:s3_cname).and_return("mycustom.domain.tld")
          @uploader.stub!(:s3_bucket).and_return("foo.bar")
          @s3_file.url.should == "http://mycustom.domain.tld/uploads/bar.txt"
        end
      end
