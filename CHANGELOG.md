...
* Fix `process convert: format` with a condition changing the file extension even when the conversion doesn't take place. Such a condition now has to be answerable from the filename, which is checked on upload (@mshibuya, [#2723](https://github.com/carrierwaveuploader/carrierwave/issues/2723))
